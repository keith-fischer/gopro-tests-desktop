...
@app.route('/testrail', methods=['POST', 'GET'])
def main():
    global callback
    global tr
    resp={}
    ...
    try:
        ...
        if _data:
            if "api" in _data and "testrail" in _data:
                _obj=_data
            else:
...
# --------------------------------------------------------------------------
# Init the testrail class which forwards requests to testrail.gopro.com web rest api service
# --------------------------------------------------------------------------
def testrail_init(data):
    global tr
    pw="Bro$toke1!"
    ...
    if "url" in data:
        url = data['url']
    tr=test_testrail.TestRail_API(url,projid,login,pw)
    if tr.ok:
        return tr.lastresponse
    tr.lastresponse["error"]="Failed to init test_testrail.TestRail_API %d %s %s %s" % (projid, login, pw, url)
    return tr.lastresponse
